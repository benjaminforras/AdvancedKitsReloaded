...
public class MainCommand extends Subcommand {
	...
	public void onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		for (int i = 0; i < kits.size(); i++)
		{
			...
			items[i - delete] = new ItemBuilder(kit.getIcon()).setTitle(ChatColor.translateAlternateColorCodes('&', kit.getDisplayName())).addLores(lore).addLores(AdvancedKits.getKitManager().getLores(player, kit)).build();
		}
		inv.setPages(items);
		inv.setTitle("Kits");
		inv.openInventory();
